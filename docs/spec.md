...
| 1 通話定額型 | `通話時間` | `128.0分`（`countedSec`） |
| 従量課金 | `通話時間` | `42.0分`（`countedSec`） |

ブロック2は全プラン共通で `通話金額` / `220円`（`Result.amount`）。

月間定額型の分子・残り枠・配色判定はいずれも `Result.quotaConsumedSec`（切り上げ後の定額枠消費量、5.4.3）を用いる。実通話時間（`countedSec`）ではないため、短い通話が多い月は実際に話した時間より大きい値が表示される。これは定額枠が実際に減る量と一致させるための意図的な仕様。1 通話定額型・従量課金は消費すべき枠が無いため実通話時間を表示する。
...
| 1 通話定額型 | `通話時間` | `128.0分`（`countedSec`） | `billedCallCount > 0` なら `超過あり`、そうでなければ `超過なし` |
| 従量課金 | `通話時間` | `42.0分`（`countedSec`） | なし（無料枠が無く「超過」が定義できないため） |
| 全プラン共通 | `通話金額` | `220円`（`amount`） | なし |

通話時間の基準は、ウィジェット（5.5.1）と数字が食い違わないよう同じ基準で選ぶ。すなわち月間定額型では `Result.quotaConsumedSec`、それ以外は枠が無いため `Result.countedSec` とする。実際に何分話したかは「履歴集計」に「通話時間（実時間）」として `countedSec` を併記する。
