...
背景は固定の色リソース（`setBackgroundResource`）ではなく、透過率（5.7）を反映した ARGB を `setBackgroundColor` で設定する。色リソース自体は不透明のまま持ち、`withAlpha()` で alpha チャンネルだけを差し替える（段階 → alpha の換算は `widgetBgAlpha()`、0 段階目 = 255、8 段階目 = 0）。透過率は上表 3 色すべてに加え、権限要求 / 更新中 / 更新失敗の各状態（5.5.2）にも同じ値を適用する。文字色（`widget_text_normal` / `widget_text_on_color`）は透過率によらず変更しないため、透過率を上げるほど壁紙の明暗によっては文字が読みにくくなる。透過率は利用者が明示的に選ぶ設定であり、読みやすさとのトレードオフは利用者に委ねる。
