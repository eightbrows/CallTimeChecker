...
| 内訳リスト | 期間内の通話一覧。日時、番号、通話時間、判定結果（定額内 / 課金 ¥XX / 除外）、課金対象秒数 |
| 権限 | 未許可時に要求ボタンを表示 |
| 操作 | 手動更新、設定画面への遷移 |
...
| 5 | 除外番号リストへの 0120（フリーダイヤル）追加 |
| 6 | 内訳リストに「課金対象のみ / 全件」フィルタ機能 |
| 7 | ウィジェットの通話時間表示を「定額枠消費量（countedSec）」ではなく「課金対象秒数（billedSec）ベース」に変更する案の検討（現状は仕様書 5.5.1 通り countedSec ベース） |