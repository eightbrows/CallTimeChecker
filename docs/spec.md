...
ウィジェット背景の透過率は、0.125 刻みの実数ではなく段階インデックス（0〜8）で保持・保存する。保存値を整数にすることで、`alpha` への換算時の端数処理を `widgetBgAlpha()` 1 箇所に閉じ込められるため。UI はプルダウン（`ExposedDropdownMenuBox`）とし、閉じている状態でも現在値（`0%` / `12.5%` / … / `100%`）を文字で読めるようにする。ラジオ 9 個は横幅に収まらず、数値入力にすると 12.5% 刻みという制約を利用者に押し付けることになるため。配置は除外番号リストの下・「保存」ボタンの上とし、他の設定項目と同じく保存対象であることが分かる並びにする。縦幅を詰めるため、項目名を左・プルダウンを右に横 1 行で並べる（プルダウンは `100%` と展開アイコンが収まる 140dp 固定幅、項目名は残り幅）。
