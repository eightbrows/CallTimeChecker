...
単一 Activity 構成。メイン画面は「サマリ」「履歴」の 2 タブ（`TabRow`）で構成する。
...
ボタンは `FilledTonalButton` とし、高さ 48dp（Material のタップターゲット推奨最小）・水平パディング 20dp・文字 `titleMedium`（16sp）とする。`TextButton` の既定サイズ（約 58x40dp・14sp）では押しづらく、背景色が無いためボタンだと気付きにくいため。ボタン 2 つで幅を使う分、アプリ名は `TextOverflow.Ellipsis` で省略して折り返さないようにする。
