...
| ウィジェット背景色（通常色 / 警告色 / 超過色） | 添字 | 0〜11 | ホワイト / オレンジ / レッド | 12 色のプリセットパレットから選ぶ（5.5.3） |
...
**ウィジェット背景色**: 通常色 / 警告色 / 超過色それぞれ 12 色のプリセットパレットから選ぶ（任意色にすると読めない配色を作れてしまうため）。保存はパレット添字（`WIDGET_COLOR_PALETTE` の並び順そのもの）。UI は現在の 3 色を矢印でつないで 1 行表示し、タップで 4 列 3 行のパレットダイアログを開く。透過率は 3 色共通。

| 添字 | 色名 | ARGB | | 添字 | 色名 | ARGB |
|---|---|---|---|---|---|---|
| 0 | ホワイト | `#FFFFFF` | | 6 | レッド | `#FF0000` |
| 1 | ティール | `#26C6DA` | | 7 | オレンジ | `#FF5722` |
| 2 | ブルー | `#0000FF` | | 8 | イエロー | `#FBC02D` |
| 3 | インディゴ | `#3F51B5` | | 9 | オリーブ | `#6B6E1E` |
| 4 | パープル | `#7B1FA2` | | 10 | グリーン | `#388E3C` |
| 5 | ピンク | `#E91E63` | | 11 | ブラック | `#000000` |
